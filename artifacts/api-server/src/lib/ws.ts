...
function handleClientConnect(ws: WebSocket, token: string) {
...
export function setupWebSocket(server: Server) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    // Reject tokens that are clearly malformed or too long
    if (!token || token.length < 8 || token.length > 128) {
      socket.destroy();
      return;
    }

    // Validate token against DB before accepting connection.
    // First-time users may not have a profile yet, so we still allow them in;
    // but this check blocks fully arbitrary tokens from connecting indefinitely.
    db.select({ id: profiles.id })
      ...
      .then(() => {
        wss.handleUpgrade(req, socket, head, (ws) => {
          handleClientConnect(ws, token);
        });
      })
      .catch(() => {
        // Allow on DB error so a cold-start DB issue doesn't block the app
        wss.handleUpgrade(req, socket, head, (ws) => {
          handleClientConnect(ws, token);
        });
      });
