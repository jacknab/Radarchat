...
import { profileEditRateLimit, goLiveRateLimit } from "../middleware/rateLimit";
...
router.put("/profile", requireToken, profileEditRateLimit, async (req: AuthedRequest, res) => {
  ...
  const incoming = {
    ...
    hosting: typeof body.hosting === "string" ? body.hosting : "",
    cockSize: typeof body.cockSize === "string" ? body.cockSize : null,
    photos: newPhotos,
    isOnline: true,
    isLive: true,
    lastSeen: now,
...
// ---------- Messages ----------
router.post("/messages", requireToken, async (req: AuthedRequest, res) => {
  ...
  const [senderProfile] = await db
    .select({ isShadowBanned: profiles.isShadowBanned })
    .from(profiles)
    .where(eq(profiles.id, senderId))
    .limit(1);

  if (senderProfile?.isShadowBanned) {
    res.status(403).json({ error: "Unable to send messages at this time." });
    return;
  }
